...
# cribbed from https://coderwall.com/p/emdlmw/skeleton-daemon-written-in-bash

daemonName="daemon-status.sh"

pidDir="/var/www/sites/earlgreyders.wang/newsite/daemon/pids"
pidFile="$pidDir/$daemonName.pid"

logDir="/var/www/sites/earlgreyders.wang/newsite/daemon/logs"
...
runInterval=120 # In seconds

doCommands() {
  # This is where you put all the commands for the daemon.
  echo "Running commands."
  php /var/www/sites/earlgreyders.wang/newsite/components/stats-update.php
}
...
startDaemon() {
  ...
  if [[ `checkDaemon` = 1 ]]; then
    echo -e " * \e[91mError\e[37m: $daemonName is already running."
    exit 1
  fi
...
stopDaemon() {
  ...
  if [[ `checkDaemon` -eq 0 ]]; then
    echo -e " * \e[91mError\e[37m: $daemonName is not running."
    exit 1
  fi
...
case "$1" in
  ...
  restart)
    ...
    ;;
  *)
  echo -e "\e[91mError\e[37m: usage $0 { start | stop | restart | status }"
  exit 1
esac
